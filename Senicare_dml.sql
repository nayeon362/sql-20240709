...
-- 요양사 테이블에 레코드를 생성 (이름, 아이디, 암호화된 비밀번호, 전화번호, 인증번호, 가입경로)
-- 필요한 추가 데이터 : 가입경로 (화면에서 가져옴), 암호화된 비밀번호 (서버가 생성)

-- 레코드 삽입시 아이디 중복 검증
SELECT * FROM nurse WHERE id = 'qwer1234';
-- 레코드 삽입시 전화번호 중복 검증
SELECT * FROM nurse WHERE tel_auth = '01012345678';
...
-- 1. 미이 인증철차가 진행중인 전화번호가 있다면 해당 전화번호 인증 레코드를 삭제 후 삽입
-- DELETE FROM tel_auth WHERE tel_number = '012345678';
...
-- 고객 테이블에서 페이지 수에 해당하는 위치부터 10개의 고객 수를 반환
...
-- 고객 테이블에서 이름이 포함되어 있는 페이지들을 번호에 해당하는 위치부터 10개의 고객 수를 반환
...
-- 고객 삭제
-- 최종 사용자 (고객 정보)
-- 고객 테이블에서 지정한 고객(고객 번호에 해당하는)의 레코드를 식별하는 작업
...
VALUES('이영희', '부산광역시','qwer1234
', null, '590826', '부산광역시 부산진구...');
...
ON CM.used_goods = G.goods_number = 1
...
SELECT * FROM customer WHERE goods_number = 1;
