...
-- 제약조건 : 데이터베이스 테이블 칼럼에 삽입, 수정, 삭제 시 적용되는 규칙
...
-- UNIQUE 제약조건 : 해당 컬럼에 중복된 데이터를 포함할 수 없도록 하는 제약조건
...
-- NOT NULL + UNIQUE = 후보키
-- 후보키 : 테이블에서 각 레코드를 고유하게 식별할 수 있응 속성
...
CREATE TABLE key_table (
    primary_colomn INT PRIMARY KEY,
...
-- FOREIGN KEY  제약조건을 특정 컬럼에 적용할 대는 반드시 데이터 타입이 참조하고자 하는 컬럼의 타입과 일치해야함.
CREATE TABLE foreign_table (
    primary1 INT PRIMARY KEY,
    foreign1 INT,
    CONSTRAINT foreign_key FOREIGN KEY(foreign1)
    REFERENCES key_table(primary_colomn)
);
...
-- FOREIGN 테이블의 컬럼에 값이 존재하지 않으면 삽입, 수정이 불가능 
UPDATE foreign_table SET foreign1 = 2 WHERE primary1 = 1;

-- FOREIGN KEY 제약조건으로 참조되어지고 있는 테이블의 레코드는 수정, 삭제가 불가능 
UPDATE key_table SET primary_colomn = 2 WHERE primary_colomn = 1;
DELETE FROM key_table WHERE primary_colomn = 1;

-- FOREIGN KEY 제약조건으로 참조되어지고 있는 테이블의 구조 변경작업이 불가능
DROP TABLE key_table;

ALTER TABLE key_table
MODIFY COLUMN primary_colomn VARCHAR(10);

-- ON UPDATE / ON DELETE 옵션
-- ON UPDATE : 참조하고 있는 테이블의 기본키가 변경될 때 동작
-- ON DELATE : 참조하고 있는 테이블의 기본키가 삭제될 때 동작

-- CASCADE : 참조되고 있는 테이블의 데이터가 삭제 또는 수정된다면, 참조하고 있는 테이블에서도 삭제 또는 수정된다면,
...
CREATE TABLE optional_foreign_table (
    primary_colomn INT PRIMARY KEY,
    foreign_colomn INT,
    FOREIGN KEY (foreign_colomn) REFERENCES key_table(primary_colomn)
    ON UPDATE CASCADE
    ON DELETE SET NULL
);
...
CREATE TABLE check_table (
    primary_colomn INT PRIMARY KEY,
    check_colomn VARCHAR(5) CHECK(check_colimn IN ('남', '여'))
);

-- CHECK 제약조건이 걸린 컬럼에 조건에 해당하지 않는 값을 삽입, 수정 할 수 없음
INSERT INTO check_table VALUES (1, '남');
INSERT INTO check_table VALUES (2, '남자');
UPDATE check_table SET check_colomn = '여자';

-- DEFAULT 제약조건 : 해당 컬럼에 삽입 시 값이 지정되지 않으면 기본값으로 지정하는 제약 
CREATE TABLE default_table (
    -- AUTO INCREMENT : 기본키가 정수형일 때 기본키의 값을 1씩 증가하는 값으로 자동 지정
    primary_colomn INT PRIMARY KEY AUTO_INCREMENT,
    colomn1 INT,
    default_colomn INT DEFAULT 10
);

INSERT INTO default_table (colomn1) VALUES (99);
...
UPDATE key_table SET primary_colomn = 2;
