...
-- Join : 특정 조건에 따라 조합하여 결과를 조회하고자 할 때 사용하는 명령어

-- INNER JOIN : 두 테이블에서 조건이 일치하는 레코드만 반환
-- SELECT column, ... FROM 기준테이블 INNER JOIN 조합할테이블
SELECT 
...
-- 만약조건에 일치하는 레코드가 존재하지 않으면 null로 반환 
...
-- 서브쿼리 : 쿼리 내부에 존재하는 또 다른 쿼리, 쿼리 겨로가를 조건이나 테이블로 사용할 수 있도록 함
...
-- FROM 절에서 서브쿼리 : 조회 결과 테이블에서 다시 FROM 절에서 재사용
